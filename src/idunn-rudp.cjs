...
async function publishIdunnRudpHealth(publisher, health) {
  ...
  try {
    ...
    const message = {
      ...
      messageId: `stonks-health:${publisher.daemonId}:${observedAt.replace(/[:.]/g, "-")}`,
      document: {
    ...
    const packets = session.sendMany("schema", wirePayload, {
      reliable: true,
      ordered: true,
      nowMs: Date.now(),
    });
...
async function receiveUntil(receiver, session, endpoint, predicate, timeoutMs, label) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (result.reply) {
        throw new Error("Stonks RUDP health publisher received an unexpected reply-required packet.");
      }
  ...
  return {
    ...
    close() {
      ...
      while (waiters.length > 0) {
        const waiter = waiters.shift();
        clearTimeout(waiter.timer);
        const error = new Error("Stonks RUDP health publisher closed.");
        error.code = "ECLOSED";
        waiter.reject(error);
      }
    },
